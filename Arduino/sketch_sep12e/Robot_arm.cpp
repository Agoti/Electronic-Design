...
//initialize
void motor_init(){
  set_servo_limit(1, 0, 1023);
	set_servo_limit(2, 0, 1023);
	set_servo_limit(3, 0, 1023);
	set_servo_limit(4, 0, 1023);
	set_servo_limit(5, 0, 1023);
  delay(1000);
}
...
void m1_left_unit(){
  ...
  //set_servo_pos(1, MAX_1 - 1, ARM_SPEED);
  //mySerial1.println("ping");
  //CyDelay(SET_LIMIT_DELAY);
	...
	if(cur_pos_1 >= MIN_1 && cur_pos_1 <= MAX_1){
		...
		set_servo_pos(1, cur_pos_1, ARM_SPEED - 30);
	}

  // mySerial1.println("pong");
	//CyDelay(100);
}
...
void m1_right_unit(){
	...
	//set_servo_limit(1, MIN_1, MAX_1);
  // set_servo_pos(1, MIN_1 + 1, ARM_SPEED);
  //CyDelay(SET_LIMIT_DELAY);
...
void pause(){
  ...
  set_servo_pos(1, cur_pos_1, 5);
	set_servo_pos(2, cur_pos_2, 5);
	set_servo_pos(3, cur_pos_3, 5);
	set_servo_pos(4, cur_pos_4, 5);
	set_servo_pos(5, cur_pos_5, 5);
  motor_init();
}
