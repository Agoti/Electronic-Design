/*******************************************************************************
* File Name: Ranging_Timer1_Clock.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: Ranging_Timer1_Clock_Start
********************************************************************************
*
* Summary:
...
void Ranging_Timer1_Clock_Start(void) 
{
    ...
    Ranging_Timer1_Clock_CLKEN |= Ranging_Timer1_Clock_CLKEN_MASK;
	Ranging_Timer1_Clock_CLKSTBY |= Ranging_Timer1_Clock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Ranging_Timer1_Clock_Stop
********************************************************************************
*
* Summary:
...
void Ranging_Timer1_Clock_Stop(void) 
{
    ...
    Ranging_Timer1_Clock_CLKEN &= (uint8)(~Ranging_Timer1_Clock_CLKEN_MASK);
	Ranging_Timer1_Clock_CLKSTBY &= (uint8)(~Ranging_Timer1_Clock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Ranging_Timer1_Clock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Ranging_Timer1_Clock_CLKEN & Ranging_Timer1_Clock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Ranging_Timer1_Clock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Ranging_Timer1_Clock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Ranging_Timer1_Clock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Ranging_Timer1_Clock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Ranging_Timer1_Clock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Ranging_Timer1_Clock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Ranging_Timer1_Clock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Ranging_Timer1_Clock_GetSourceRegister();
    uint16 oldDivider = Ranging_Timer1_Clock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Ranging_Timer1_Clock_CLKEN & Ranging_Timer1_Clock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Ranging_Timer1_Clock_DIV_PTR, clkDivider);
                Ranging_Timer1_Clock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Ranging_Timer1_Clock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Ranging_Timer1_Clock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Ranging_Timer1_Clock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(Ranging_Timer1_Clock_DIV_PTR, clkDivider);
				Ranging_Timer1_Clock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: Ranging_Timer1_Clock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Ranging_Timer1_Clock_DIV_PTR);
}


/*******************************************************************************
* Function Name: Ranging_Timer1_Clock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Ranging_Timer1_Clock_MOD_SRC |= modeBitMask & (uint8)Ranging_Timer1_Clock_MODE_MASK;
}


/*******************************************************************************
* Function Name: Ranging_Timer1_Clock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Ranging_Timer1_Clock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Ranging_Timer1_Clock_MODE_MASK));
}


/*******************************************************************************
* Function Name: Ranging_Timer1_Clock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Ranging_Timer1_Clock_MOD_SRC & (uint8)(Ranging_Timer1_Clock_MODE_MASK);
}


/*******************************************************************************
* Function Name: Ranging_Timer1_Clock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Ranging_Timer1_Clock_GetDividerRegister();
    uint8 oldSrc = Ranging_Timer1_Clock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Ranging_Timer1_Clock_MOD_SRC |= CYCLK_SSS;
        Ranging_Timer1_Clock_MOD_SRC =
            (Ranging_Timer1_Clock_MOD_SRC & (uint8)(~Ranging_Timer1_Clock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Ranging_Timer1_Clock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Ranging_Timer1_Clock_MOD_SRC & Ranging_Timer1_Clock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Ranging_Timer1_Clock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Ranging_Timer1_Clock_PHASE = clkPhase & Ranging_Timer1_Clock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Ranging_Timer1_Clock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Ranging_Timer1_Clock_PHASE & Ranging_Timer1_Clock_PHASE_MASK;
}
