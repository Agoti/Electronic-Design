/*******************************************************************************
* File Name: Ranging_Timer2_Clock.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: Ranging_Timer2_Clock_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Ranging_Timer2_Clock_Start(void) 
{
    /* Set the bit to enable the clock. */
    Ranging_Timer2_Clock_CLKEN |= Ranging_Timer2_Clock_CLKEN_MASK;
	Ranging_Timer2_Clock_CLKSTBY |= Ranging_Timer2_Clock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Ranging_Timer2_Clock_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Ranging_Timer2_Clock_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    Ranging_Timer2_Clock_CLKEN &= (uint8)(~Ranging_Timer2_Clock_CLKEN_MASK);
	Ranging_Timer2_Clock_CLKSTBY &= (uint8)(~Ranging_Timer2_Clock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Ranging_Timer2_Clock_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Ranging_Timer2_Clock_StopBlock(void) 
{
    if ((Ranging_Timer2_Clock_CLKEN & Ranging_Timer2_Clock_CLKEN_MASK) != 0u)
    {
...
#if defined(Ranging_Timer2_Clock__CFG3)
        CLK_DIST_AMASK = Ranging_Timer2_Clock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Ranging_Timer2_Clock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Ranging_Timer2_Clock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Ranging_Timer2_Clock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Ranging_Timer2_Clock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Ranging_Timer2_Clock_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Ranging_Timer2_Clock_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        Ranging_Timer2_Clock_CLKSTBY &= (uint8)(~Ranging_Timer2_Clock_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: Ranging_Timer2_Clock_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Ranging_Timer2_Clock_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = Ranging_Timer2_Clock_GetSourceRegister();
    uint16 oldDivider = Ranging_Timer2_Clock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Ranging_Timer2_Clock_CLKEN & Ranging_Timer2_Clock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Ranging_Timer2_Clock_DIV_PTR, clkDivider);
                Ranging_Timer2_Clock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Ranging_Timer2_Clock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Ranging_Timer2_Clock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Ranging_Timer2_Clock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(Ranging_Timer2_Clock_DIV_PTR, clkDivider);
				Ranging_Timer2_Clock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: Ranging_Timer2_Clock_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 Ranging_Timer2_Clock_GetDividerRegister(void) 
{
    return CY_GET_REG16(Ranging_Timer2_Clock_DIV_PTR);
}


/*******************************************************************************
* Function Name: Ranging_Timer2_Clock_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Ranging_Timer2_Clock_SetModeRegister(uint8 modeBitMask) 
{
    Ranging_Timer2_Clock_MOD_SRC |= modeBitMask & (uint8)Ranging_Timer2_Clock_MODE_MASK;
}


/*******************************************************************************
* Function Name: Ranging_Timer2_Clock_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Ranging_Timer2_Clock_ClearModeRegister(uint8 modeBitMask) 
{
    Ranging_Timer2_Clock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Ranging_Timer2_Clock_MODE_MASK));
}


/*******************************************************************************
* Function Name: Ranging_Timer2_Clock_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Ranging_Timer2_Clock_GetModeRegister(void) 
{
    return Ranging_Timer2_Clock_MOD_SRC & (uint8)(Ranging_Timer2_Clock_MODE_MASK);
}


/*******************************************************************************
* Function Name: Ranging_Timer2_Clock_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Ranging_Timer2_Clock_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = Ranging_Timer2_Clock_GetDividerRegister();
    uint8 oldSrc = Ranging_Timer2_Clock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Ranging_Timer2_Clock_MOD_SRC |= CYCLK_SSS;
        Ranging_Timer2_Clock_MOD_SRC =
            (Ranging_Timer2_Clock_MOD_SRC & (uint8)(~Ranging_Timer2_Clock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Ranging_Timer2_Clock_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Ranging_Timer2_Clock_GetSourceRegister(void) 
{
    return Ranging_Timer2_Clock_MOD_SRC & Ranging_Timer2_Clock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Ranging_Timer2_Clock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Ranging_Timer2_Clock_SetPhaseRegister(uint8 clkPhase) 
{
    Ranging_Timer2_Clock_PHASE = clkPhase & Ranging_Timer2_Clock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Ranging_Timer2_Clock_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Ranging_Timer2_Clock_GetPhaseRegister(void) 
{
    return Ranging_Timer2_Clock_PHASE & Ranging_Timer2_Clock_PHASE_MASK;
}
